...
const AdminDashboard = () => {
  ...
  const { currentUser, isSuperAdmin, isWardAdmin, logout, getUserWard, DAMAK_TOTAL_WARDS } = useAuth();
  ...
  // Admin info from auth context
  const admin = {
    name: currentUser?.fullName || "Admin User",
    role: isSuperAdmin() ? t.superAdmin : t.wardAdmin,
    ward: isWardAdmin() ? `${t.ward} ${getUserWard()}, Damak` : "Damak Municipality",
  };
  ...
  // Menu items - different for super admin vs ward admin
  const menuItems = isSuperAdmin()
    ? [
        { id: "dashboard", icon: Home, label: t.dashboard },
        { id: "wardManagement", icon: UserPlus, label: t.wardManagement },
        { id: "issues", icon: FileText, label: t.issues, badge: stats.pending },
        { id: "users", icon: Users, label: t.users, badge: stats.pendingKyc },
        { id: "notifications", icon: Megaphone, label: t.notifications },
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {isSuperAdmin() && (
            <div className="flex items-center gap-2">
              ...
              >
                <option value="all" className="text-gray-800">{t.allWards}</option>
