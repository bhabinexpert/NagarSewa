      ...
      <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <img
            src="/nagarsewa.jpg"
            alt="NagarSewa logo"
            className="h-11 w-11 rounded-full object-cover ring-2 ring-emerald-200 shadow transition-transform duration-300 hover:scale-110"
          />
          <div>
        ...
        <nav className="flex items-center gap-4">
          <div className="hidden lg:flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-sm font-medium">
            {navItems.map((item) => (
              <a
                key={item.key}
                href={item.href}
